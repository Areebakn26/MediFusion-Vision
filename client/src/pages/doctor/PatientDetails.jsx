...
const PatientDetails = () => {
    ...
    if (loading) return <div className="h-screen flex items-center justify-center bg-slate-50"><div className="animate-spin rounded-full h-12 w-12 border-b-4 border-primary-blue"></div></div>;
    ...
    return (
        <div className="space-y-10 p-4 md:p-8 bg-slate-50 min-h-screen font-sans">
            {/* Minimalist Navigation */}
            <div className="flex items-center justify-between">
                <Button 
                    variant="outline" 
                    onClick={() => navigate('/doctor/patients')} 
                    className="flex items-center gap-3 border-2 border-slate-200 text-slate-500 font-black text-xs tracking-widest rounded-2xl px-6 py-3 hover:bg-white transition-all"
                >
                    <FaArrowLeft /> BACK TO ROSTER
                </Button>
                <div className="flex gap-4">
                    <Button onClick={() => navigate('/doctor/appointments')} className="bg-primary-blue hover:bg-blue-700 text-white font-black text-xs tracking-widest rounded-2xl px-8 py-3 shadow-xl shadow-blue-500/20">
                        SCHEDULE SESSION
                    </Button>
                </div>
            </div>
            ...
            <div className="relative group">
                ...
                <GlassCard className="relative p-10 bg-white border-none shadow-2xl rounded-[3rem] overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row gap-12 items-center">
                        <div className="relative">
                            <img 
                                ...
                                className="w-40 h-40 rounded-[3rem] shadow-2xl border-8 border-slate-50 object-cover"
                            />
                        ...
                        <div className="flex-1 text-center md:text-left space-y-4">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-6 text-[11px] font-black uppercase tracking-widest text-slate-400">
                                <span className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                                    <FaUser className="text-primary-blue" /> {profile.gender} • {profile.date_of_birth ? (new Date().getFullYear() - new Date(profile.date_of_birth).getFullYear()) : 'N/A'} YRS
                                </span>
                                <span className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                                    <FaEnvelope className="text-purple-500" /> {profile.User?.email}
                                </span>
                                <span className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                                    <FaPhone className="text-teal-500" /> {profile.User?.phone || 'NO PHONE'}
                                </span>
                            </div>
                        </div>
                        <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-800 shadow-2xl min-w-[240px] text-center">
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.4em] mb-3">Hematology Group</p>
                            <p className="text-5xl font-black text-white italic tracking-tighter">{profile.blood_group || 'O+'}</p>
                        </div>
                    </div>
                </GlassCard>
            </div>

            <TabGroup tabs={tabs} defaultTab="timeline">
                {(activeTab) => (
                    <div className="mt-12">
                        {activeTab === 'timeline' && (
                            <div className="space-y-8 max-w-4xl mx-auto">
                                {appointments.length === 0 ? (
                                    <div className="py-20 text-center flex flex-col items-center border-4 border-dashed border-slate-100 rounded-[4rem]">
                                        <FaHistory className="text-slate-100 text-8xl mb-6" />
                                        <p className="text-xs font-black text-slate-300 uppercase tracking-widest italic">No clinical history with this facility</p>
                                    </div>
                                ) : (
                                    appointments.map((apt, idx) => (
                                        ...
                                        >
                                            <GlassCard className="p-10 border-none shadow-xl rounded-[3rem] hover:shadow-2xl transition-all relative overflow-hidden">
                                                ...
                                                >
                                                    <div className="space-y-4">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <h3 className="font-black text-2xl text-slate-900 tracking-tighter uppercase italic">
                                                    ...
                                                    <div className="flex flex-col items-end gap-3">
                                                        ...
                                                        <span className="text-[9px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2 cursor-pointer">
                                                    ...
                                                    <div className="mt-8 pt-8 border-t border-slate-100 space-y-8">
                                                        ...
                                                        {apt.consultation_summary && (() => {
                                                            ...
                                                            if (summary && typeof summary === 'object') {
                                                                return (
                                                                    <div>
                                                                        <h4 className="text-[10px] font-black text-purple-600 uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
                                                                            <div className="w-2 h-2 bg-purple-500 rounded-full"></div> AI SCRIBE — SESSION SUMMARY
                                                                        </h4>
                                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                            {[
                                                                                { label: 'Chief Complaint', key: 'chiefComplaint', color: 'bg-red-50 border-red-100' },
                                                                                { label: 'Clinical Assessment', key: 'assessment', color: 'bg-slate-50 border-slate-100' },
                                                                                { label: 'Treatment Plan', key: 'plan', color: 'bg-blue-50 border-blue-100' },
                                                                                { label: 'History of Present Illness', key: 'historyOfPresentIllness', color: 'bg-slate-50 border-slate-100' },
                                                                                { label: 'Follow-Up Instructions', key: 'followUpInstructions', color: 'bg-green-50 border-green-100' },
                                                                            ].filter(f => summary[f.key]).map(f => (
                                                                                <div key={f.key} className={`${f.color} rounded-2xl p-5 border`}>
                                                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">{f.label}</p>
                                                                                    <p className="text-sm font-bold text-slate-800 italic leading-relaxed">{summary[f.key]}</p>
                                                                                </div>
                                                                            ))}
                                                                            {summary.symptoms?.length > 0 && (
                                                                                <div className="bg-orange-50 rounded-2xl p-5 border border-orange-100">
                                                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Identified Symptoms</p>
                                                                                    <div className="flex flex-wrap gap-2">
                                                                                        {summary.symptoms.map((s, i) => (
                                                                                            <span key={i} className="bg-orange-200 text-orange-900 text-[10px] font-black px-3 py-1 rounded-xl uppercase">{s}</span>
                                                                                        ))}
                                                                                    </div>
                                                                                </div>
                                                                            )}
                                                                        ...
                                                                        {summary.patientSummary && (
                                                                            <div className="mt-4 p-6 bg-purple-50 rounded-2xl border border-purple-100">
                                                                                <p className="text-[9px] font-black text-purple-600 uppercase tracking-widest mb-2">Patient-Facing Summary</p>
                                                                                <p className="text-sm text-purple-900 font-bold italic leading-relaxed">"{summary.patientSummary}"</p>
                                                                            </div>
                                                                        )}
                                                                    </div>
                                                                );
                                                            ...
                                                            return (
                                                                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Consultation Summary</p>
                                                                    <p className="text-sm text-slate-700 leading-relaxed italic">{apt.consultation_summary}</p>
                                                                </div>
                                                            );
                                                        })()}
                                                        ...
                                                        {apt.ConsultationNotes?.length > 0 && (
                                                            <div>
                                                                <h4 className="text-[10px] font-black text-primary-blue uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
                                                                    <div className="w-2 h-2 bg-primary-blue rounded-full"></div> SESSION NOTES ({apt.ConsultationNotes.length})
                                                                </h4>
                                                                <div className="space-y-3 max-h-80 overflow-y-auto pr-2" style={{ scrollbarWidth: 'thin' }}>
                                                                    {apt.ConsultationNotes.map((note) => (
                                                                        <div key={note.id} className={`rounded-2xl p-5 border ${note.is_private ? 'bg-amber-50 border-amber-100' : 'bg-blue-50 border-blue-100'}`}>
                                                                            <div className="flex justify-between items-center mb-2">
                                                                                <div className="flex items-center gap-2">
                                                                                    <span className={`text-[9px] font-black uppercase tracking-widest ${note.is_private ? 'text-amber-600' : 'text-blue-600'}`}>
                                                                                        {note.note_type || 'clinical'} {note.is_private ? '• PRIVATE' : ''}
                                                                                    </span>
                                                                                    <span className="text-[9px] text-slate-400">— {note.author?.name || 'Doctor'}</span>
                                                                                </div>
                                                        ...
                                                        {!apt.consultation_summary && !apt.ConsultationNotes?.length && (
                                                            <p className="text-center text-sm text-slate-400 italic py-4">No notes recorded for this session.</p>
                                                        )}
                                                    </div>
                                                )}
                                            </GlassCard>
                        ...
                        {activeTab === 'scans' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                                {scans.length === 0 ? (
                                    <div className="col-span-full py-20 text-center flex flex-col items-center border-4 border-dashed border-slate-100 rounded-[4rem]">
                                        <FaImage className="text-slate-100 text-8xl mb-6" />
                                        <p className="text-xs font-black text-slate-300 uppercase tracking-widest italic">No diagnostic imaging found</p>
                                    </div>
                                ) : (
                                    scans.map((scan, idx) => (
                                        <motion.div
                                        ...
                                        >
                                            <GlassCard 
                                                className="group rounded-[3rem] border-none shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 cursor-pointer"
                                            ...
                                            >
                                                <div className="relative aspect-square bg-black overflow-hidden">
                                                    ...
                                                    <div className="absolute bottom-6 left-6 space-y-2">
                                                        <Badge className="bg-white/10 backdrop-blur-xl text-white border-white/20 text-[9px] uppercase font-black px-4 py-1.5 tracking-[0.2em]">
                                                ...
                                                <div className="p-8 bg-white space-y-6">
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <FaRobot className={scan.status === 'analyzed' ? 'text-purple-600' : 'text-slate-200'} size={18} />
                                                    </div>
                                                    
                                                    {scan.status === 'analyzed' ? (
                                                        <div className="space-y-4">
                                                            <div className="p-4 bg-purple-50 rounded-2xl border border-purple-100">
                                                                <p className="text-[9px] font-black text-purple-700 uppercase tracking-widest mb-1">AI Classification</p>
                                                            ...
                                                            {scan.Report && (
                                                                <div 
                                                                    className="p-4 bg-blue-50 rounded-2xl border border-blue-100 hover:bg-blue-100 transition-colors"
                                                                    onClick={(e) => { e.stopPropagation(); setSelectedReport(scan.Report); }}
                                                                >
                                                                    <div className="flex items-center justify-between mb-2">
                                                                        <span className="text-[9px] font-black text-blue-800 uppercase tracking-widest">Final Clinical Report</span>
                                                                        <FaPlus size={10} className="text-blue-500" />
                                                                    </div>
                                                                    <p className="text-xs font-bold text-blue-900 line-clamp-1 italic">{scan.Report.diagnosis}</p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    ) : (
                                                        <Button 
                                                            variant="outline" 
                                                            className="w-full py-4 rounded-2xl text-[10px] font-black tracking-widest border-2 border-slate-100 text-slate-400 hover:bg-slate-50"
                        ...
                        {activeTab === 'profile' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto">
                                <GlassCard className="p-10 rounded-[3rem] border-none shadow-xl space-y-8">
                                    <h3 className="text-xl font-black text-slate-900 tracking-tighter uppercase italic border-b border-slate-50 pb-6 flex items-center gap-4">
                                        <FaUser className="text-primary-blue" /> BIOMETRIC PROFILE
                                    </h3>
                                    <div className="space-y-6">
                                        {[
                                            { label: 'Hematology Group', value: profile.blood_group || 'O+' },
                                            { label: 'Biological Gender', value: profile.gender },
                                            ...
                                            { label: 'Emergency Contact', value: profile.emergency_contact || 'None Recorded' }
                                        ].map((item, i) => (
                                            <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.label}</span>
                                                <span className="text-sm font-black text-slate-800 italic">{item.value}</span>
                                            </div>
                                        ))}
                                    </div>
                                </GlassCard>
                                ...
                                <GlassCard className="p-10 rounded-[3rem] border-none shadow-xl space-y-8">
                                    <h3 className="text-xl font-black text-slate-900 tracking-tighter uppercase italic border-b border-slate-50 pb-6 flex items-center gap-4">
                                        <FaNotesMedical className="text-red-500" /> CHRONIC CONDITIONS
                                    </h3>
                                    <div className="space-y-4">
                                        {profile.MedicalHistories?.length > 0 ? (
                                            profile.MedicalHistories.map((h, i) => (
                                                <div key={i} className="p-6 bg-red-50/50 rounded-[2rem] border border-red-100 relative group overflow-hidden transition-all hover:bg-red-50">
                                                    <div className="absolute top-0 right-0 p-6 opacity-5">
                                                        <FaHeartbeat size={48} className="text-red-600" />
                                                    </div>
                                                    <div className="flex justify-between items-start mb-2">
                                                        <h4 className="text-base font-black text-red-900 uppercase tracking-tight">{h.condition}</h4>
                                                        <Badge variant="danger" className="text-[8px] px-3 py-1 font-black">{h.status}</Badge>
                                                    </div>
                                                    <p className="text-xs text-red-700/70 font-bold italic">{h.notes || 'No specific contraindications recorded.'}</p>
                                                </div>
                                            ))
                                        ) : (
                                            <div className="py-12 text-center border-2 border-dashed border-slate-100 rounded-[2rem]">
                                                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">No clinical history recorded</p>
                                            </div>
                                        )}
                                    </div>
                                </GlassCard>
                            ...
                            <div className="bg-slate-900 p-10 rounded-[4rem] border border-slate-800 shadow-2xl relative overflow-hidden">
                                ...
                                {getAIConfidence(selectedScan.ai_prediction) != null && (
                                    <div className="mt-8 flex items-center gap-6">
                                        <div className="flex-1 h-3 bg-slate-800 rounded-full overflow-hidden border border-slate-700">
                                            <div className="h-full bg-gradient-to-r from-purple-600 to-primary-blue" style={{ width: `${getAIConfidence(selectedScan.ai_prediction)}%` }}></div>
                                        </div>
                                ...
                                <div className="mt-6">
                                    <Button
                                        onClick={() => handleRunAI(selectedScan.id)}
                                        disabled={isProcessingAI}
                                        className="w-full bg-purple-600/20 hover:bg-purple-600/40 text-purple-300 font-black py-3 rounded-2xl border border-purple-500/30"
                                    >
                ...
                {selectedReport && (
                    <div className="bg-white p-4 space-y-10">
                        <div className="flex justify-between items-center p-6 bg-slate-900 rounded-[2rem] text-white">
                            <div>
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Verification Date</p>
                                <p className="text-base font-black italic">{new Date(selectedReport.createdAt).toLocaleDateString(undefined, { dateStyle: 'full' })}</p>
                            </div>
                            <Badge variant="success" className="px-6 py-2 font-black uppercase tracking-[0.3em] text-[10px] bg-green-500 border-none shadow-xl shadow-green-500/20 text-white">VERIFIED</Badge>
                        </div>
                        <div className="space-y-10 px-4">
                            ...
                            {selectedReport.recommendations && (
                                <section className="p-8 bg-blue-50 rounded-[3rem] border border-blue-100 relative">
                                    <h5 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] mb-4">RECOMMENDATIONS</h5>
